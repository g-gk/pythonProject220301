...
def main():
    ...
    wl2 -= "Andromeda"
    print(wl2, wl2.get_destinations(), sep="\n")

    print('Пример 2')
    wl = WaitingLounge("Zet", ["Sirius", "Antares"], 750)
    wl1 = WaitingLounge("Beta", ["Polaris"], 1500)
    print(wl >= wl1, wl == wl1, wl < wl1)
    print(wl(5), wl1(7))
    print(wl, wl.get_destinations(), sep="\n")
    res = wl * 4
    res[0] -= "Betelgeuse"
    print(res)
    print(*[item.get_destinations() for item in res])
